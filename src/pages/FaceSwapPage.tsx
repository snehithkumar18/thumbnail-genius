...
const FaceSwapPage = () => {
  ...
  const saveFace = async () => {
    ...
    try {
      ...
      const path = `faces/${user.id}/${faceId}.png`;
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      if (customTargetFile && activeTargetUrl === customTarget) {
        const path = `targets/${user.id}/${crypto.randomUUID()}.png`;
                ...
                <TabsContent value="my" className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                    {filteredThumbnails?.map((t) => (
                      <div
                        key={t.id}
                        onClick={() => { setSelectedTarget(t.image_url); setCustomTarget(null); }}
                      ...
                      >
                        <img
                          ...
                          alt=""
                          className={`w-full object-cover ${t.format_type === "9:16" ? "h-24" : "h-16"}`}
                        />
                      </div>
                    ))}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      {SWAP_STRENGTHS.map((s) => (
                        <button
                          key={s.value}
                          onClick={() => setSwapStrength(s.value)}
                          className={`flex-1 text-center py-2 px-3 rounded-lg text-sm transition-all ${
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <img src={activeTargetUrl || ""} alt="Before" className="w-full rounded-lg" />
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 text-center">After</p>
                      <img src={result} alt="After" className="w-full rounded-lg" />
                    </div>
                  </div>
