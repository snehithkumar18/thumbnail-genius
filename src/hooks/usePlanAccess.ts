...
export const usePlanAccess = () => {
  ...
  const planStatus = (credits as any)?.plan_status ?? 'none';
  const subscriptionCredits = (credits as any)?.subscription_credits ?? 0;
  const topupCredits = (credits as any)?.topup_credits ?? 0;
