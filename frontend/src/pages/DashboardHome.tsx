...
const DashboardHome = () => {
  ...
  const quickActions = [
    ...
    { label: "Smart Edit", icon: Sparkles, route: "/dashboard/smart-editor", emoji: "✨" },
  ];
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((action, i) => {
            const isSmartEdit = action.label === "Smart Edit";
            return (
              <motion.div
                key={action.label}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <Button
                  variant="outline"
                  className={`w-full h-24 flex flex-col items-center justify-center gap-1.5 transition-all ${
                    isSmartEdit 
                      ? "border-[#D4C8FF] bg-gradient-to-br from-[#F0E8FF] to-[#EDE9FE] hover:border-[#8B47FF] hover:shadow-[0_8px_20px_rgba(139,71,255,0.2)]"
                      : "border-border hover:border-primary/30 hover:bg-primary/5"
                  }`}
                  onClick={() => navigate(action.route)}
                >
                  {isSmartEdit ? (
                    <Sparkles className="h-6 w-6 text-[#8B47FF] mb-0.5" />
                  ) : (
                    <span className="text-2xl">{action.emoji}</span>
                  )}
                  <span className={`text-sm font-medium ${isSmartEdit ? "text-[#0F0A1E]" : "text-foreground"}`}>{action.label}</span>
                  {isSmartEdit && <span className="text-[10px] text-[#8B47FF]/80 font-medium">Edit layer by layer</span>}
                </Button>
              </motion.div>
            )
          })}
