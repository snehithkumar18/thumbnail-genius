      ...
      if (logoFile) {
        ...
        try {
          ...
          if (!bgError && bgData?.image_url) {
            finalLogoUrl = bgData.image_url;
          }
        } catch {
          // Keep original upload if background removal is unavailable.
        }
      }
          ...
          <Card className="border-border bg-card overflow-hidden">
            <div className="p-4 border-b border-border bg-muted/30">
               <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                  <Palette className="h-4 w-4 text-primary" /> Live Brand Preview
               </h3>
               <p className="text-[10px] text-muted-foreground mt-1 uppercase tracking-wider">How your branding looks on real thumbnails</p>
            </div>
            <CardContent className="p-5 space-y-6">
              {/* Mockup thumbnails */}
              {[
                { text: "GENERATE VIRAL CONTENT", img: "https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=400&h=225&fit=crop" },
                { text: "MY NEW SETUP TOUR", img: "https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=400&h=225&fit=crop" },
              ].map((mock, i) => (
                <div
                  key={i}
                  className="relative aspect-video rounded-xl overflow-hidden shadow-2xl group"
                  style={{
                    border: frameStyle === "thick-border" ? `4px solid ${primaryColor}` : undefined,
                    boxShadow: frameStyle === "neon-glow" ? `0 0 20px ${primaryColor}40` : undefined,
                  }}
                >
                  <img src={mock.img} alt="" className="w-full h-full object-cover grayscale-[0.2]" />
                  
                  {/* Neon Glow effect */}
                  {frameStyle === "neon-glow" && (
                     <div className="absolute inset-0 ring-4 ring-inset ring-white/10" style={{ boxShadow: `inset 0 0 30px ${primaryColor}80` }} />
                  )}
                  ...
                  {frameStyle === "gradient-overlay" && (
                    <div
                      ...
                      style={{
                        background: `linear-gradient(to top, ${primaryColor}90, transparent)`,
                      }}
                    />
                  )}
                  ...
                  {/* Corner badge (Logo) */}
                  {logoUrl && (
                    <div className={`absolute w-10 h-10 p-1.5 bg-white/10 backdrop-blur-md rounded-lg shadow-lg ${frameStyle === "corner-badge" ? "top-3 right-3" : "bottom-3 left-3"}`}>
                      <img src={logoUrl} alt="" className="w-full h-full object-contain filter drop-shadow-md" />
                    </div>
                  )}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center p-6">
                    <span
                      className={`text-center leading-tight drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)] ${FONT_STYLES.find((f) => f.id === fontStyle)?.preview.replace('text-lg', 'text-2xl') || "font-black text-2xl"}`}
                      style={{ 
                        color: textColor,
                        WebkitTextStroke: frameStyle === "neon-glow" ? `1px ${primaryColor}` : undefined
                      }}
                    >
                      {mock.text}
                    </span>
              ...
              <div className="bg-primary/5 rounded-lg p-3 border border-primary/10">
                 <p className="text-[10px] text-primary font-medium text-center">
                    These styles will be applied automatically to every thumbnail you generate.
                 </p>
              </div>
            </CardContent>
          </Card>
